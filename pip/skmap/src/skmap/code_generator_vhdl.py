    ...
    def vhdl_type_str(self) -> str:
        ...
        if self.vec_len is None:
            match self.kind:
                ...
                case ValueKindVhdl.slv       : return f"std_ulogic_vector{elem_rng}"
                case ValueKindVhdl.unsigned  : return f"unsigned{elem_rng}"
                case ValueKindVhdl.signed    : return f"signed{elem_rng}"
                case ValueKindVhdl.character : return f"character"
...
def op_to_vhdl_source(op : BultiInOperation) -> str:
    return {
...
def to_vhdl_source(node: ResolvableT) -> str:
    """Render a ResolvableT tree back out as a vhdl source-code expression string."""
...
def ipkg_type_to_vhdl_str(t : ValueType, port_types:PortTypes='all') -> str:
    ...
    if port_types == 'flat':
        if t.is_vec:
            vec_flat_rng = f'({to_vhdl_source(t.vec_len)}*{to_vhdl_source(t.elem_w)}-1 downto 0)'
...
def vhdl_skmap_map_acc_byte_inc_func_str(r : RecipeRegFw, k : bool) -> str:
    s = (f"    skmap_map_acc_byte_inc(byte_idx, VAL_W=>{to_vhdl_source(r.t.elem_w)}")
    if not k:
...
package {recipe.fw_module}_ipkg is\n\n""")
        ...
        for ipkgv in recipe.ipkg:
            vhdl_t = ipkg_type_to_vhdl_str(ipkgv.t, port_types='all')
            value = ipkgv.value
...
""")
  ...
  function get_{kv.name} return boolean_vector is
    variable flags : boolean_vector(0 to {kv.t.elem_w}-1) := (others => false);
  begin\n""")
    ...
    # variable offset_v : integer := 0;
        ...
        for kv in recipe_k_fw:
            if not kv.t.kind.is_int:
                ...
                # else:
                #     if ((addr_byte + kv.t.elem_w//8) % 4) == 0:
                #         if kv.t.elem_w == 8:
...
###################################
  ...
  function get_REGS_VAR_WR_INIT return integer_vector is
    variable vec_int_0to : integer_vector(0 to REGS_VAR_LEN-1) := (others => 0);
    alias vec_int : integer_vector(HEAD_AND_K_LEN to HEAD_AND_K_LEN+REGS_VAR_LEN-1) is vec_int_0to(0 to REGS_VAR_LEN-1);
    variable byte_idx : integer := HEAD_AND_K_LEN*4;
  begin\n""")
            # variable offset_v : integer := 0;
            # for ii in range(total_k_with_fixed_idx):
            #     kv = recipe.k[ii]
            ...
            for varv in recipe_var_fw:
                if varv.fw_init == FwInitMode.zero:
                    ...
                    is_signed_str = 'TRUE' if varv.t.kind == ValueKindVhdl.integer else 'FALSE'
                    vhdl_f.write(f"    skmap_map_acc_k(k_vec_int_io=>vec_int, byte_idx_io=>byte_idx, val_i=>{varv.init_name}, w_i=>{to_vhdl_source(varv.t.elem_w)}, signed_i=>{is_signed_str}, {align_str});\n")
            ...
            vhdl_f.write(f"""    return vec_int_0to;
  end function;
...
##################################
  ...
  # begin\n""")
  #       for varv in recipe.var:
  #           vhdl_f.write(f"    skmap_map_acc_byte_inc(byte_idx, VAL_W=>{varv.t.elem_w},")
...
\n""")
        for kv in recipe_k_fw:
            if kv.t.kind in (ValueKindVhdl.boolean, ValueKindVhdl.boolean_vector, ValueKindVhdl.sl, ValueKindVhdl.slv):
                continue
            if kv.t.is_vec:
                # TODO: add type checking for vectors
                continue
            if kv.t.kind == ValueKindVhdl.unsigned:
                if not isinstance(kv.t.elem_w, int) or kv.t.elem_w < 31:
                    vhdl_f.write(f'  assert {kv.name} < 2**{kv.t.elem_w} severity FAILURE;\n')
                else:
                    assert kv.t.elem_w <= 31, "Not yet supported"
            elif kv.t.kind == ValueKindVhdl.signed:
                if not isinstance(kv.t.elem_w, int) or kv.t.elem_w < 32:
                    vhdl_f.write(f'  assert {kv.name} <   2**({kv.t.elem_w}-1) severity FAILURE;\n')
                    vhdl_f.write(f'  assert {kv.name} >= -2**({kv.t.elem_w}-1) severity FAILURE;\n')
                else:
                    assert kv.t.elem_w <= 32, "Not yet supported"
...
""")
        ...
        for varv in recipe_var_fw:
            if varv.uses_var_name:
                ...
                vhdl_t = varv.t.vhdl_type_str_slv_2d()
    ...
    byte_idx_v := HEAD_AND_K_LEN*4;\n\n""");
        ...
        for varv in recipe_var_fw:
            ...
            if not varv.acc.sw_writable:
                if varv.t.is_vec:
                    match varv.t.kind:
                        case ValueKindVhdl.natural: assert False
                        case ValueKindVhdl.integer: assert False
                        case ValueKindVhdl.sl: rd_name =  varv.name_ext
                        case ValueKindVhdl.slv: rd_name =  varv.name_ext
                        case ValueKindVhdl.unsigned: rd_name =  f"to_vec_slv({varv.name_ext})"
                        case ValueKindVhdl.signed  : rd_name =  f"to_vec_slv({varv.name_ext})"
                        case ValueKindVhdl.character: assert False
                        case ValueKindVhdl.string: assert False

                    assert rd_name is not None
                else:
                    match varv.t.kind:
                        case ValueKindVhdl.natural: assert False
                        case ValueKindVhdl.integer: assert False
                        case ValueKindVhdl.sl: rd_name =  f"to_slv({varv.name_ext})"
                        case ValueKindVhdl.slv: rd_name =  varv.name_ext
                        case ValueKindVhdl.unsigned: rd_name =  f"std_ulogic_vector({varv.name_ext})"
                        case ValueKindVhdl.signed  : rd_name =  f"std_ulogic_vector({varv.name_ext})"
                        case ValueKindVhdl.character: assert False
                        case ValueKindVhdl.string: assert False
                    assert rd_name is not None
                ...
                if varv.direction == "out":
                    ...
                    if varv.flags is None:
                        if port_types == 'flat':
                            ...
                            match varv.t.kind:
                                case ValueKindVhdl.natural: vhdl_f.write(f'    {varv.p_name} <= unsigned({varv.name_ext});\n')
                                case ValueKindVhdl.integer: vhdl_f.write(f'    {varv.p_name} <= signed({varv.name_ext});\n')
                                case _: assert False;
                    else:
                        assert( varv.flags is not None )
